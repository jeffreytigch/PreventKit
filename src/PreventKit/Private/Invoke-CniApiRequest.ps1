<#
.SYNOPSIS
POST a request body to the MDE Custom Network Indicators API with an
Authorization Bearer token and 429 rate-limit backoff and retry.

.DESCRIPTION
Sends a POST to the given API Uri with the body serialized as JSON, carrying
the caller-supplied token as an Authorization Bearer header. When the request
fails with HTTP 429 (Too Many Requests), it backs off for an increasing number
of seconds and retries, up to MaxRetries attempts, keeping the token on every
attempt. Any other failure is rethrown immediately.
